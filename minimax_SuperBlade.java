...
public class minimax_SuperBlade extends AIModule
{
	...
	public void getNextMove(final GameStateModule game)
	{
		...
		if (game.getCoins()==0)
			...
			chosenMove = MiniMaxSearch(state);
	}
	
	private int MiniMaxSearch(StateType state)
	{		
		state.Succesors = Succesors(state);
		startLevel = state.MoveCount;
		
		int j = MaxValue(state);
	...
	private int MaxValue(StateType state)
	{
		...
		for (Integer a : succ.keySet()) {	//search the hashmap of the current states to get the maximum one.
			state.V = Math.max(state.V, MinValue(succ.get(a)));
		}
		return state.V;
	}
	
	private int MinValue(StateType state)
	{
		if (CutOff(state)) return Eval(state.Values);
		state.V = Integer.MAX_VALUE;
		HashMap<Integer, StateType> succ = (state.Succesors != null) ? state.Succesors : Succesors(state);
		for (Integer a : succ.keySet()){
			state.V = Math.min(state.V, MaxValue(succ.get(a)));
		}
		return state.V;
	}
